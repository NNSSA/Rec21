...
def create_mask(input_array):
    ...
    # define mask
    mask = np.ones(input_array.shape, dtype=int)
...
if __name__ == "__main__":
    input_array = np.load("T21s_z=15.0_128Mpc_32Cells_1boxes_norelvel_psi=0.9.npy")[0]
    input_array_fft = np.fft.fftn(input_array)
    mask = create_mask(input_array_fft)
